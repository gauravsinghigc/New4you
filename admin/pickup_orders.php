          ...
          <div class="users-list-table">
            <div class="card">
              <div class="card-header">
                <h4 class="users-action">All Orders <i class="fa fa-angle-right"></i>
                  <span> Delivery On :
                    <?php echo date("D d M, Y", strtotime("+1 days")); ?></span><br>
                  <hr>
                  <a href="export_orders.php" class="btn btn-sm btn-primary"><i class="fa fa-file-pdf-o"></i> OFFLINE ORDER
                    SHEET</a>
                  <a href="items.php" class="btn btn-sm btn-primary"><i class="fa fa-list"></i>
                    STOCK PURCHASE</a>
                  <a href="print_bills.php" class="btn btn-sm btn-primary"><i class="fa fa-print"></i> PRINT BILLS</a>
                  <a href="rejected_orders.php" class="btn btn-sm btn-primary"><i class="fa fa-warning"></i> Rejected Orders</a>
                </h4>
                      ...
                      <tbody>
                        ...
                        if ($user_role == "SUPER_ADMIN") {
                          ...
                          if (isset($_GET['order_type'])) {
                            ...
                            $sql = "SELECT * FROM customer_orders, customers where customer_orders.store_id='$store_id' and customers.customer_id=customer_orders.customer_id and customer_orders.order_status!='DELIVERED' and customer_orders.order_status!='REJECTED' ORDER BY customer_orders.customer_order_id DESC";
                            $query = mysqli_query($con, $sql);
                          }
                        }
                        ...
                        if ($count == 0) {
                          echo "<tr align='center'>
                                                        <td colspan='8'><h3><i class='fa fa-warning'></i><br></h3><h3>No Orders Available!</h3>
                                                    </tr>";
                        } else {
                          while ($fetch = mysqli_fetch_assoc($query)) {
                            ...
                            <tr>
                              <td style="padding: 1%; font-size: 12px;">
                                <h4><a href="pickup_deliver.php?id=<?php echo $fetch['order_id']; ?>" class='text-info' style="padding: 1%; font-size: 12px;"><?php echo $fetch['order_id']; ?></a>
                                </h4>
                              </td>
                              <td style="padding: 1%; font-size: 12px;">
                              ...
                              <td style="padding: 1%; font-size: 12px;width: 130px;">
                                ...
                                <?php if ($DELIVERY_TYPE == "STORE_PICKUP") { ?>
                                  <?php if ($order_status == "NEW_ORDER") { ?>
                                    <a href="update.php?accept_id=<?php echo $order_id; ?>&store_id=<?php echo $store_id; ?>&cr_url=<?php echo get_url(); ?>" class='btn btn-info btn-sm float-left'>
                                      Accept
                                    </a>
                                    <a href="update.php?reject_id=<?php echo $order_id; ?>&store_id=<?php echo $store_id; ?>&sms=&s_p=&s_m=&cr_url=<?php echo get_url(); ?>" class='btn btn-warning btn-sm float-right'>
                                      Reject
                                    </a>
