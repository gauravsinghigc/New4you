...
} else if (isset($_GET['address_remove'])) {
	$address_id = $_GET['address_remove'];
	$customer_id = $_GET['cid'];
	$sql = "DELETE FROM customer_address where customer_address_id='$address_id' and customer_id='$customer_id'";
	$query = mysqli_query($con, $sql);
	if ($query == true) {
		header("location: checkout.php?data=Address Deleted!");
	} else {
		header("location: checkout.php?err=Unable to Delete Address");
	}
}
